  ...
  return (
    <>
        ...
        <div onClick={() => handleSetIndex(index)}
         className={`cursor-pointer	flex w-5/6 md:w-1/2 justify-between p-2 mt-2 rounded bg-black ${(index===activeIndex)?"animated":""}`}>
            <div className='flex'>
        ...
        {(activeIndex === index) && (
            <div  className={`shadow-3xl rounded-2xl shadow-cyan-500/50 flex w-5/6 md:w-1/2 justify-between p-2 mt-2  ${(index===activeIndex)?"animated":""}`}>
              {children}
            </div>
        )}
    </>
